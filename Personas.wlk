...
class Personas {
	...
	method quiereEntrarEnCarpa(carpa){
		return self.leGustaCerveza(carpa.marca()) and (self.leGustaTradicional() and carpa.bandaMusical())
	}
...
class Belga inherits Personas {
	...
	override method esPatriota(){
		return jarrasTomadas.all({ jarra => jarra.pais() == "Belga"})
	}
}
...
class Checo inherits Personas {
	...
	override method esPatriota(){
		return jarrasTomadas.all({ jarra => jarra.pais() == "Checo"})
	}
}
